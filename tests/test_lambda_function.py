...
@pytest.mark.parametrize("target, actual, exp_msg, exp_color", [
    (1, 1, "クリティカル", COLOR_CRITICAL),
    (5, 5, "クリティカル", COLOR_CRITICAL),
    (20, 15, "成功", COLOR_SUCCESS),
    (100, 100, "成功", COLOR_SUCCESS),
    (1, 5, "失敗", COLOR_FAILURE),
    (94, 95, "失敗", COLOR_FAILURE),
    (95, 96, "ファンブル", COLOR_FUMBLE),
...
def test_split_alternative_roll_or_value(cmd, result):
    assert split_alternative_roll_or_value(cmd) == result
...
def test_eval_roll_or_value_for_roll(text, count, min_val, max_val):
    results = eval_roll_or_value(text)
...
@pytest.mark.parametrize("cmd, san_val, msg_matcher, color", [
    ("SANC", 100, r"成功 【SANチェック】 \d+/100", COLOR_SUCCESS),
    ("SANC", 0, r"失敗 【SANチェック】 \d+/0", COLOR_FAILURE),
    ("SANC 1/1D3", 100,
     r"成功 【SANチェック】 \d+/100\n【減少値】 \d", COLOR_SUCCESS),
    ("SANC 1/1D3", 0,
     r"失敗 【SANチェック】 \d+/0\n【減少値】 \d", COLOR_FAILURE),
...
def test_get_sanc_result(cmd, san_val, msg_matcher, color):
    actual_msg, actual_color = get_sanc_result(cmd, san_val)
...
def test_create_post_message_rolls_result(text, message, detail, sum_val):
    result_message, resule_detail, result_sum = create_post_message_rolls_result(text)
...
def test_analyze_update_command(command, exp_status_name, exp_operator, exp_arg):
    result = analyze_update_command(command)
    assert result
